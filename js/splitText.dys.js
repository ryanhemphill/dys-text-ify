...
(function($) {
  ...
  function cleanUpContent(thisTarget) {
    // if(!isJquery(thisTarget)) { thisTarget = $(thisTarget); }
    // thisTarget.find('.white-space').replaceWith(' ');
    // var lines = thisTarget.find('.line');
    // lines.each(function(index) {
    //   thisLine = $(this);
    //   thisLine.removeClass('current');
    //   if(thisLine.text() == '') { thisLine.detach(); }
    // });
  }
...
// (function() {
//   $.fn.eachInverted = function(passedFunction) {
//     var allElements = $(this);
//     allElements.each(index) {
//       var thisIndex = allElements.length-1-index;
//       var thisElement = allElements.eq(thisIndex);
//       thisElement
//     }
